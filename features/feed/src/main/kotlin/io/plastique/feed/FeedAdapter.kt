...
private class CollectionUpdateItemDelegate(
    ...
    override fun onBindViewHolder(item: CollectionUpdateItem, holder: ViewHolder, position: Int, payloads: List<Any>) {
        ...
        holder.binding.folderName.text = holder.binding.root.resources.getString(R.string.feed_collection_folder_name, item.folderName, item.addedCount)
        holder.folderItemsAdapter.update(item.folderItems)
    }

    override fun onViewAttachedToWindow(holder: RecyclerView.ViewHolder) {
        super.onViewAttachedToWindow(holder)
        (holder as ViewHolder).run { holder.binding.items.layoutManager = layoutManager }
    }

    override fun onViewDetachedFromWindow(holder: RecyclerView.ViewHolder) {
        super.onViewDetachedFromWindow(holder)
        (holder as ViewHolder).run { holder.binding.items.layoutManager = null }
    }
    ...
    class ViewHolder(
        ...
        val layoutManager: FlexboxLayoutManager = FlexboxLayoutManager(binding.items.context)
...
private class UsernameChangeItemDelegate(
    ...
    override fun onBindViewHolder(item: UsernameChangeItem, holder: ViewHolder, position: Int, payloads: List<Any>) {
        holder.binding.header.time = elapsedTimeFormatter.format(item.date)
        holder.binding.header.setUser(item.user, imageLoader)
        holder.binding.description.text = HtmlCompat.fromHtml(holder.binding.root.resources.getString(R.string.feed_username_change_description,
